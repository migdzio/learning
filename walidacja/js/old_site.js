...
// Po kliknięciu w "Wyslij" funkcja ma sprawdzić wszystkie pola formularza, jesli się któreś nie zgadza to ramka danego inputa
...
function validate() {
	var wrongEmail = document.forms["formValidation"]["email"].style.border = "2px solid #FF0C02" ;
	var lastName = document.forms["formValidation"]["lastName"].value;
	var emailVal = document.forms["formValidation"]["email"].value;
	var onlyLettersAndSpaces = /^[a-zZ-a]{1}[a-ząćśńółęźżĄĘŹŻĆ \.]{0,30}/i;
	var testSurname = onlyLettersAndSpaces.test(lastName);
	...
	if(testSurname){
		...
		document.forms["formValidation"]["lastName"].style.border = "2px solid #FF0C02" ;
	}
